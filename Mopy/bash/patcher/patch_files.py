...
class PatchFile(_PFile, ModFile):
    ...
    def mergeModFile(self,modFile,progress,doFilter,iiMode):
        ...
        for blockType,block in modFile.tops.iteritems():
            iiSkipMerge = iiMode and blockType not in bush.game_mod.listTypes
...
class CBash_PatchFile(_PFile, ObModFile):
    ...
    def buildPatch(self,progress):
        ...
        levelLists = bush.game_mod.listTypes
        nullProgress = Progress()
        ...
        if self.Current.LookupModFileLoadOrder(self.patchName.temp.s) <= 0:
            print (u'\n'.join((_(u"Please copy this entire message and report it on the current official thread at http://forums.bethsoft.com/index.php?/forum/25-mods/."),
                   _(u'Also with:'),
                   _(u'1. Your OS:'),
