...
class IniFile(AFile):
    ...
    reDeletedSetting = re.compile(u';-\\s*(\\w.*?)\\s*(;.*$|=.*$|$)',re.U)
    reSection = re.compile(u'^\\[\\s*(.+?)\\s*\\]$',re.U)
    reSetting = re.compile(u'(.+?)\\s*=(.*)',re.U)
...
class OBSEIniFile(IniFile):
    ...
    reSetNGS   = re.compile(u'\\s*SetNumericGameSetting\\s+(.+?)\\s+(.*)', re.I|re.U)
