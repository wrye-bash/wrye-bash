...
class MasterInfo:
    ...
    def flag_fallback(self, pflag):
        ...
        mysterious _was_esl."""
        if pflag is bush.game.esm_flag:
...
class _TabledInfo:
    ...
    def get_persistent_attrs(self, exclude=None):
        if exclude is None:
...
class FileInfo(_TabledInfo, AFileInfo):
    ...
    def revert_backup(self, first): # single call site - good
        ...
        # refresh below will try to refresh info sets but we don't back up
        # the config so we can't really detect changes in imported/merged -
        # a (another) backup edge case - as backup is half-baked anyway
        # let's agree for now that BPs remain BPs with the same config as
        # before - if not, manually run a mergeability scan after updating
        # the config (in case the restored file is a BP)
        self._store().refresh(
            {self.fn_key: {'att_val': self.get_persistent_attrs()}})
...
class ModInfo(FileInfo):
    ...
    _is_master = _is_esl = _is_overlay = _is_mid = _is_blueprint = False
    ...
    def set_plugin_flags(self, flags_dict: dict[PluginFlag, bool | None],
        ...
        for pl_flag, flag_val in flags_dict.items():
            ...
            if flag_val is not None and pl_flag is bush.game.esm_flag:
    ...
    def _update_onam(self):
        ...
        if bush.game.Esp.generate_temp_child_onam:
            if bush.game.esm_flag.cached_type(self):
...
class TableFileInfos(_AFileInfos):
    ...
    def save_pickle(self):
        ...
        for k, v in self.items():
            if pickle_dict := v.get_persistent_attrs(exclude=frozenset()):
...
class ModInfos(TableFileInfos):
    ...
    def rescanMergeable(self, names, progress=bolt.Progress(),
        ...
        with progress:
            ...
            result = {}
            for i, fileName in enumerate(names): # todo sort
    ...
    def cached_lo_last_esm(self):
        ...
        for mod in self._lo_wip[1:]:
            if not bush.game.esm_flag.cached_type(self[mod]):
                return last_esm
            last_esm = mod
        return last_esm
    ...
    def _lo_move_mod(self, old_name, new_name, do_activate, deactivate=False):
        ...
        self._lo_wip[oldIndex:oldIndex] = [new_name]
