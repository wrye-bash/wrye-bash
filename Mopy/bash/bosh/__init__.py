...
class _AFileInfos(DataStore):
    ...
    def refresh(self, refresh_infos: bool | RefrIn = True, *, booting=False):
        ...
        except AttributeError: # XXX debug
            if not isinstance(refresh_infos, bool):
                raise ValueError(f'Unexpected type for {refresh_infos=}')
        ...
        with os.scandir(self.store_dir) as it: # performance intensive
            for x in it:
                try:
                    ...
                    deprint(f'Failed to stat {x.name} in {self.store_dir}', #XXX debug
                            traceback=True)
