...
class _RecGroupDict(dict):
    ...
    def __missing__(self, top_grup_sig, __rh=RecordHeader):
        ...
        if top_grup_sig in __rh.top_grup_sigs:
            topClass = self._mod_file.loadFactory.getTopClass(top_grup_sig)
            try:
                self[top_grup_sig] = topClass(
                    TopGrupHeader(0, top_grup_sig, 0, 0),
                    self._mod_file.loadFactory)
            except TypeError:
                raise ModError( ##: hmm -> KeyError ?
                    self._mod_file.fileInfo.name,
