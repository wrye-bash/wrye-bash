...
def generateTweakLines(wizardEdits, target):
    ...
    for realSection, values in wizardEdits.items():
        ...
        for realSetting in values[1]:
            ...
            lines.append(fmt % {'setting': setting, 'value': value,
                                'comment': comment})
    return lines
    ...
    def Continue(self):
        ...
        while self.cLine < len(self.lines):
            ...
            try:
                ...
                return PageError(self._wiz_parent, _('Installer Wizard'),
                     '\n'.join([_('An error occurred in the wizard script:'),
                     _('Line %s:\t%s') % (self.cLine, newline.strip('\n')),
                     _('Error:\t%s') % e]))
            except Exception:
                bolt.deprint('Error while running wizard', traceback=True)
                msg = '\n'.join([_('An unhandled error occurred while '
                    'parsing the wizard:'), _('Line %s:\t%s') % (self.cLine,
