...
class AppButton(AppLauncher, StatusBarButton):
    ...
    def sb_click(self, *, custom_args: tuple[str, ...] = ()):
        exeargs = self.app_cli(custom_args)
        Link.Frame.set_status_info(shlex.join([self._app_path.s, *exeargs]))
        try:
            self.launch_app(self.app_path, exeargs)
            return
        except UnicodeError:
