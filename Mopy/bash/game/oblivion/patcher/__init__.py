...
_strU = struct.Struct(u'I').unpack
def _fid(_x): return None, _x # None <=> game master
def _cobl(_x): return u'Cobl Main.esm', _x

class _OblivionPatchGame(PatchGame):
    # Need to locally import the checkers/preservers since those files need to
    # use bush.game, which won't be set when we initialize the game yet
    @property
    def gameSpecificPatchers(self):
    ...
    @property
    def gameSpecificListPatchers(self):
    ...
    actor_importer_attrs = {
        b'CREA': {
            u'Actors.ACBS': (u'barterGold', u'baseSpell', u'calcMax', u'calcMin',
                             ...
                             u'flags.weaponAndShield', u'level'),
