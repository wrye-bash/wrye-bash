...
def generateTweakLines(wizardEdits, target):
    lines = []
    lines.append(_(u'; Generated by Wrye Bash %s for \'%s\' via wizard') % (basher.settings['bash.readme'][1], target.s))
    for realSection in wizardEdits:
        if realSection == u']set[':
            format = wizardEdits[realSection][0]+u' %(setting)s to %(value)s%(comment)s'
        elif realSection == u']setgs[':
            format = wizardEdits[realSection][0]+u' %(setting)s %(value)s%(comment)s'
        elif not realSection:
            continue
        else:
            lines.append(u'')
            lines.append(u'[%s]' % wizardEdits[realSection][0])
            format = u'%(setting)s=%(value)s%(comment)s'
...
class WryeParser(ScriptParser.Parser):
    ...
    def fnEditINI(self, iniName, section, setting, value, comment=None):
        ...
        self.iniedits[iniPath].setdefault(realSection,[u'',{}])
        self.iniedits[iniPath][realSection][0] = section
        self.iniedits[iniPath][realSection][1][realSetting] = (setting,value,comment)
