...
class LoadOrder(object):
    ...
    def lo_diff(self, other: LoadOrder):
        ...
        count = collections.Counter({k for k, _v in diff})
        ldiff.act_reordered = {k for k, c in count.items() if
                               k not in new_del and c == 2}
        ldiff.active_flips = {k for k, c in count.items() if
                              k not in new_del and c == 1}
        ldiff.act_del = (ldiff.active_flips | ldiff.missing) & self.active
        ldiff.act_new = (ldiff.active_flips | ldiff.added) & other.active
        return ldiff
...
def refresh_lo(cached: bool, cached_active: bool):
    ...
    **must be up to date** for correct load order/active validation."""
    ...
    if locked and _saved_load_orders:
        ...
        if _cached_lord is not __lo_unset:
            if _cached_lord != saved: # sanity check, should not happen
                bolt.deprint(f'Bug: {_cached_lord=} is different from {saved=}')
    ...
    if locked and saved is not __lo_unset:
        if _cached_lord.loadOrder != _cached_lord.loadOrder or (
