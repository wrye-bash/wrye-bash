...
class Installer_Wizard(InstallerLink):
    ...
    def Execute(self, event):
        ...
        for iniFile in ret.IniEdits:
            outFile = bosh.dirs['mods'].join('INI Tweaks','Wizard Tweak [%s].ini'%iniFile.sbody)
            file = outFile.open('w')
            ...
            iniList.RefreshUI()
            if iniFile in installer.data_sizeCrc:
                # Editing an INI from this installer, so go ahead and apply it
